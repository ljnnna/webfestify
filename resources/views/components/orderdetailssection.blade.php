@php
    $items = $cartItems ?? ($paymentData['cart_items'] ?? []);
@endphp

@foreach($items as $item)
@php
    // Hitung rental_days jika tidak ada langsung di variabel
    $rental_days = isset($item->rental_days)
        ? $item->rental_days
        : (\Carbon\Carbon::parse($item->start_date)->diffInDays(\Carbon\Carbon::parse($item->end_date)) + 1);

    // Ambil detail pengiriman dari kolom JSON jika perlu
...
{{-- Kode HTML kamu tetap seperti ini --}}
