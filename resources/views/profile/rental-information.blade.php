...
@foreach ($activeOrders as $order)
    @php
        ...
        // ✅ PERBAIKAN DISINI: utamakan status sewa, bukan hanya status pembayaran
        if ($order->status) {
            ...
            $adjustedStatus = 'pending'; // fallback default
        }
    ...
    <div class="mt-4 {{ $bgColor }} rounded-lg shadow-md border-l-4 {{ $borderColor }} flex p-4 space-x-4 relative">
        <img src="{{ $product->images->first() ? asset('storage/' . $product->images->first()->path) : asset('images/no-image.jpg') }}"
             alt="{{ $product->name }}"
        ...
        <div class="flex-1 flex flex-col justify-between">
            <div>
                ...
                <div class="mt-2 text-gray-600 text-sm space-y-1">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span>
                            Method: <span class="capitalize">{{ $order->delivery_option ?? 'N/A' }}</span>
                        </span>
                    </div>
            ...
            <div class="flex space-x-3 mt-4 justify-end">
                ...
                @if ($adjustedStatus === 'confirmed')
                <a href="{{ route('profile.deliveryTracking', ['order' => $order->id]) }}"
                   class="text-white text-xs px-4 py-1 rounded bg-blue-400 hover:bg-blue-600">
                    Lihat Status Pengantaran
                </a>
                @endif
